;*---------------------------------------------------------------------------
;  :Program.	GulpHD.asm
;  :Contents.	Slave for "Gulp"
;  :Author.	JOTD
;  :Original	v1 
;  :Version.	$Id: GulpHD.asm 1.2 2002/02/08 01:18:39 wepl Exp wepl $
...
; this complex stuff is just to emulate JSR (A4)
; but flush the caches first (and be able to patch some
; code, although it doesn't seem necessary right now)

run_subprogram_cd32:
    bsr _flushcache
    ; emulate jsr from trap
...
.afterrte:
	...
	move.l	a4,-(a7)
    cmp.l   #$33FCC020,($288,a4)
    bne.b   .no_language_am
    movem.l D0-D1/A0-A2,-(a7)
    move.l  _resload(pc),a2
    lea pl_language_cd32(pc),a0
    move.l  a4,a1
...
run_subprogram_floppy:
	move.l	a0,-(a7)
    move.l  $68.W,a0
    cmp.w   #$48E7,(a0)
    beq.b   .interrupt_ok
    ; re-install default keyboard interrupt
    ; as the game sometimes loads parts where
    ; keyboard interrupt isn't set, so vector points to
    ; the old address, where code is something else => crash
    ; here routine doesn't start by a MOVEM.L regs,-(a7)
    ; install our own
...
.afterrte:
	move.l	jsr_address(pc),-(a7)
	move.l	a4,-(a7)    ; launch address
    cmp.l   #$103900bf,($6310-$1F02,a4)
    bne.b   .no_main
    movem.l D0-D1/A0-A2,-(a7)
    move.l  _resload(pc),a2
    ...
    movem.l (a7)+,D0-D1/A0-A2
.no_main
    cmp.l   #$227afbf2,($4620,a4)
    bne.b   .no_prot
    movem.l D0-D1/A0-A2,-(a7)
    move.l  _resload(pc),a2
    lea pl_prot_floppy(pc),a0
    move.l  a4,a1
    jsr (resload_Patch,a2)
    movem.l (a7)+,D0-D1/A0-A2
.no_prot
...
pl_main_floppy
    PL_START
    PL_PS   $6322-$1F02,kbint_hook
    PL_END
pl_prot_floppy
    PL_START
    PL_B   $6496-$1F02,$60  ; protection no longer appears
    ;PL_B   $650E-$1F02,$60  ; answer always ok
    PL_PS   $2cca-$1F02,kbint_hook
    PL_END
  
kbint_hook:
    cmp.b   _keyexit(pc),d0
    beq   _quit
	MOVE.W	D0,D1			;86322: 3200
	ANDI.W	#$007f,D1		;86324: 0241007f    
    rts
    
pl_language_cd32:
    PL_START
    PL_ORW  $8218C-$81F02,8 ; enable keyboard interrupt
    PL_END
    
pl_cd32
	PL_START
    PL_PA   $7FE,keyboard_interrupt
	PL_P	$C64,decrunch
    PL_W    $003fa,$4E4F
    
    ; leave keyboard interrupt on
    PL_ORW  $6E2,8
    PL_ORW  $764,8
    
    ; get rid of OS completely
    PL_PS   $107E,read_file
    PL_S    $1084,$dc-$84
    PL_S    $010E2,$10F4-$10E2
    PL_NOP  $00710,4        ; no dos delay
    PL_R    $0084c          ; no cache hack
    PL_S    $A,$D4-$A       ; skip openlibs
    
    PL_PS   $e0,get_mem_chunk_size
    PL_S    $e6,$114-$e6
    PL_PSS  $126,alloc_chunk,2
	PL_END
    
pl_floppy
	PL_START
    PL_PA   $007f8,keyboard_interrupt
	PL_P	$00c88,decrunch
    PL_W    $003de,$4E4F
    ...
    ; get rid of OS completely
    PL_S    $007b0,$C6-$B0    ; remove VBR relocate
    PL_PS   $010a2,read_file
    PL_S    $010a8,$dc-$84
    PL_S    $01106,$10F4-$10E2
    PL_NOP  $006f4,4        ; no dos delay
    PL_R    $00870          ; no cache hack
    PL_S    $A,$B8-$A       ; skip openlibs
    
    PL_PS   $000c4,get_mem_chunk_size
    PL_S    $000ca,$000f8-$ca
    PL_PSS  $0010a,alloc_chunk,2
    
    PL_P    $00752,_quit        ; quit when ESCaping from end sequence
	PL_END    
