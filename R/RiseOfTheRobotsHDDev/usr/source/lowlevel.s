...
;
; if the CD of the game includes lowlevel.library file in "LIBS", it is strongly
; recommended to use it instead of this shitty code. However, the fakepad.s source
; should be used in order to make believe the controller is a joypad (protection in
; some games, which refuse to run without joypad, such as James Pond 2)
;
; the other difference is that mouse is not supported in ReadJoyPort,
; but on the other hand, if a joystick is connected, then the call always tell
...
.port0
	move.l	#JP_TYPE_GAMECTLR,D0	; type of controller is joypad, always

	moveq	#CIAB_GAMEPORT0,d3	; 1rst button (left mouse)
	moveq	#10,d4			; 2nd button (right mouse)
	move.w	#$F600,d5
	lea	(joy0dat,a5),a3		; joystick 1 moves
	bsr.b	.port_test
.rjexit
	movem.l	(sp)+,d1-d6/a3-a5
	rts
...
.port1
	...
	move.w	#$6F00,d5
	lea	(joy1dat,a5),a3		; joystick 2 moves
...
.no_red_button
	...
	IFEQ	1
	move.l	vbi_counter(pc),d2
.lll
	cmp.l	vbi_counter(pc),d2
	beq.b	.lll

	move.w	(potinp,a5),d2		; a5=$DFF016
	move.w	#$CC01,(potgo,a5)	; a5=$DFF034
	btst	d4,d2
	bne.b	.no_blue_button
	bset	#JPB_BUTTON_BLUE,d0	; fire/lmb
.no_blue_button
	bra.b	.out
	ENDC
...
; < a3: custom reg. of the selected joystick
