...
;
; actually, the info I had in 2014 from EAB / Toni "Winuae" Wilen was

; History:
;
; - 2018: JOTD force controller type port 0: mouse, port 1: joypad, can be changed
;   from inside the slave by writing to port_0/1_attribute
...
; autosense doesn't work very well... force it
port_0_attribute
	dc.l	JP_TYPE_MOUSE   ; SJA_TYPE_AUTOSENSE
port_1_attribute
	dc.l	JP_TYPE_GAMECTLR
...
.joypad_mode
	...
	moveq	#17-1,d1	; was done 24 times, not needed, just shift in the end
...
.read_ok
	; now the bits match lowlevel interface
	lsl.l	#7,d6
...
.noplay
	...
	; combine joypad output with keys
	move.l	D0,-(A7)
	movem.l	D1/D2,-(A7)
	move.l	.old_buttonmask(pc),D0
	bsr	.fake_button_test
	movem.l	(A7)+,D1/D2
...
.autosense
	move.l	d7,d3
	bne.b	.some_inputs
	; d7 can be 0 in which case we cannot detect now
	; also means that there are no movements, assume joypad
...
.query_array:
JOYPAD1KEYS:
	dc.w	$54,0	; 0:F5: fire 2/blue/rmb
	dc.w	$55,0   ; 4:F6: Green
	dc.w	$56,0   ; 8:F7: Yellow
	dc.w	$57,0   ; 12:F8: Play/pause
	dc.w	$58,0   ; 16:F9: left ear
	dc.w	$59,0   ; 20:F10:right ear
JOYPAD0KEYS:
	dc.w    $5,0
	dc.w    $6,0
	dc.w    $7,0
