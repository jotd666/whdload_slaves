...
; joystick/joypad and an unstable A1200, which did not help at all :)
;
...
; History:
; - 2020: JOTD fixed joypad/joystick detection, added API to prefer joystick to mouse in autosense
; - 2019: JOTD fixed joypad/joystick read which was completely broken except for 2nd button
...
; < D0: port number
; < D1: controller type SJA_TYPE_MOUSE or SJA_TYPE_JOYSTK

set_autosense_hint:
    movem.l d0/a0,-(a7)
    and.w   #1,d0
    lea _autosense_hint(pc),a0
    move.b  d1,(a0,d0.w)
    movem.l (a7)+,d0/a0
    rts
    
; mouse in port 0, joystick in port 1 if gamepad isn't detected
_autosense_hint:
    dc.b    SJA_TYPE_MOUSE,SJA_TYPE_JOYSTK
    
_port_attributes
	dc.l	SJA_TYPE_AUTOSENSE,SJA_TYPE_AUTOSENSE
...
.port0
	...
	bsr.b	.port_test
    moveq.l #0,d1
    ; correct controller type if needed
.rjexit
    bsr .correct_controller_type
...
;;;;port_attributes
	...
	moveq	#9,d1	; must be done 9 times, allowing to read & detect controller type

	IFD	_BUTTONS_DEBUG
	move.l	#$55555555,$10C	; debug, start of loop
	ENDC

	bra.b	.lbC000746	; removed: shaving timer issue too close
						; 2017: reinstated (identical as Wepl timing fix)
                        ; 2020: exact same code as lowlevel library official code

.button_loop
    tst.b   ciapra(a4)      ; yeah ciapra is 0...
    tst.b   ciapra(a4)
    tst.b   ciapra(a4)
.lbC000746
    tst.b   ciapra(a4)
    ...
    tst.b   ciapra(a4)
    tst.b   ciapra(a4)
	move.w	(a0),d2		; a0=$DFF016
	bset	d3,(a4)                      ;Make the clock high
	bclr	d3,(a4)                       ;Lower the clock.
	btst	d4,d2
	bne.b	.lbC00077C
    ;Buttons are active low.
	bset	d1,d6   ;The button was pressed.
.lbC00077C
	dbra	d1,.button_loop
	; acknowledge port input first
	move.w	#$FF00,(potgo,a5)	; a5=$DFF034	; correct value according to robinsonb5@eab

	; all buttons have been read, reset data direction register to input
	bclr	d3,(ciaddra,a4)
    ...
    ; Toni: Last 2 bits (bits 7 and 8) from CD32 pad should be zero followed by one
    ; (or was it one followed by zero?) and current button state does not affect them. So if all received bits are all zero or all one: it can't be a CD32 pad. 
    
	IFD	_BUTTONS_DEBUG
	move.l	D6,$108.W	; DEBUG	
	ENDC
	
    ; d6 values depending on controller type:
    ; CD32 joypad:
    ; - nothing pressed:   00000003
    ; - something pressed: xxxxxxx3
    ; joystick/mouse:
    ; - nothing pressed:   00000000
    ; - something pressed: xxxxxxxF
    ; (red button doesn't count)
	...
	IFD	_BUTTONS_DEBUG
	move.l	#$C0DEDBAD,$10C	; debug
	ENDC
	; button 2 of the joystick/mouse must be read again
.not_a_joypad
    ; hack/speedup: no need to read button 2 again: if D4 is 0, it's not pressed
    ; else it's pressed
    tst.b   d4
    beq.b   .no_blue
...
.read_ok
 	IFD	_BUTTONS_DEBUG
	move.l	#$C001C001,$10C	; debug
	ENDC
   ; this is a real CD32 controller
	lsl.l	#8,d6	; correct bits offsets
	lsl.l	#6,d6	; correct bits offsets
	; all button reads are valid
	and.l	#JP_BUTTON_MASK,d6
    ; this can't be a mouse, this is a game controller
    ; because all buttons could be read
    ; (that's also how lowlevel library does to be sure, whereas it
    ; cannot be sure about joystick or mouse unless it reads several times)
...
.out
	...
	move.l	(A7)+,D0
	IFD	_BUTTONS_DEBUG
	move.l	D0,$100.W	; debug
	move.l	#$DEADBEEF,$104.W
	ENDC
.rptexit
    IFD _BUTTONS_DEBUG
...
; < D1: port number
; trashes D3,D2,D1,A0
.correct_controller_type
   
    move.l  d0,d2
    and.l   #JP_TYPE_MASK,d2
    ; if it is a controller, never mind autosense
    cmp.l   #JP_TYPE_GAMECTLR,d2
    beq   .nomouse    ; directly read directions
    ...
    move.l  (a0,d1.w),d3
    movem.l (a7)+,d1
    cmp.l   #SJA_TYPE_AUTOSENSE,d3
    beq.b   .sja_autosense
    and.l   #JP_BUTTON_MASK,d0
    ; apply type
    cmp.l   #SJA_TYPE_MOUSE,d3
    bne.b   .sja_nomouse
    or.l    #JP_TYPE_MOUSE,d0
    bra.b   .out_ctas
.sja_nomouse
    cmp.l   #SJA_TYPE_JOYSTK,d3
    bne.b   .sja_nojoy
    or.l    #JP_TYPE_JOYSTK,d0
    bra.b   .out_ctas
.sja_nojoy    
.sja_autosense
    lea    _autosense_hint(pc),a0
...
.set_ctype
    ...
    swap    d2
    clr.w   d2
    or.l    d2,d0   ; apply controller "hint" (actually manual hint, much better than trying to autodetect)
.out_ctas
    ; step to avoid returning JOYSTK as a type!
    move.l  d0,d2
    and.l   #JP_TYPE_MASK,d2
    cmp.l   #JP_TYPE_JOYSTK,d2
    bne.b   .nojoy
    ; NEVER set TYPE_JOYSTK as most games refuse to work with it
    ; whether red+blue works with 2 button joys and all buttons can be
    ; emulated by keypresses using function keys or other configurable keys
    move.l  #JP_TYPE_GAMECTLR,d2
    and.l   #JP_BUTTON_MASK,d0
    or.l    d2,d0
.nojoy
    ; now that all autosense stuff is set, read directions or just joydat value depending on controller type
    cmp.l   #JP_TYPE_MOUSE,d2
    bne.b   .nomouse
    lea _custom+joy0dat,a0
    add.w   d1,a0   ; port
    add.w   d1,a0
    or.w    (a0),d0 ; mouse readings, raw
    bra.b   .out_ct
.nomouse
    ; joypad/joystick: read directions
    lea _custom+joy0dat,a0
    add.w   d1,a0   ; port
    add.w   d1,a0
